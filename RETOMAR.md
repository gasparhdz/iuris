...
- **Deploy oficial:** push a `main` → GitHub Actions (`.github/workflows/deploy.yml`). Build en el runner, artefactos por rsync al VPS, git pull + migraciones + restart. **NUNCA buildear en el VPS** (tsc y vite OOMean con 2 GB). Plan B manual: `powershell -File scripts\deploy.ps1`. Secrets requeridos en el repo: `VPS_SSH_KEY`, `VPS_KNOWN_HOSTS`.
