...
- **Deploy oficial (MANUAL):** GitHub → Actions → "Deploy production" → **Run workflow** (`.github/workflows/deploy.yml`, solo `workflow_dispatch`). Pushear a `main` NO despliega — se puede pushear libremente para sincronizar entre PCs. El workflow buildea en el runner, sube artefactos por rsync, corre migraciones y reinicia con healthcheck. **NUNCA buildear en el VPS** (tsc y vite OOMean con 2 GB). Plan B: `powershell -File scripts\deploy.ps1`. Secrets del repo: `VPS_SSH_KEY`, `VPS_KNOWN_HOSTS`.
